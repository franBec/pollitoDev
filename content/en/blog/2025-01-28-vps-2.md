...
<!-- TOC -->
  ...
  * [Choose an OS](#choose-an-os)
    * [What is an OS with panel?](#what-is-an-os-with-panel)
...
After obtaining the instance, it is time to choose an OS: The popular choice would be just Ubuntu, but for this blog series we are going to go with an OS with Panel -> [Coolify](https://www.coolify.io/).

![hostinger-select-os](/uploads/2025-01-28-vps-2/hostinger-select-os.png)

### What is an OS with panel?
A standard OS (like plain Ubuntu) gives you a blank server to configure via the command line. But an OS with a panel bundles a graphical control panel (like Coolify) that lets you manage your server through a simple web interface.
- Think of it like a dashboard for your VPS: you can deploy apps, monitor resources, configure settings, and handle updates without needing to memorize terminal commands.
- Panels are perfect for developers who want to skip the command-line grind and focus on building things.
...
- Also add an SSH Key so you can later log in exclusively through SSH.
...
Ideally now you would want to disable root login to lock down your server. 
