...
<!-- TOC -->
  * [Elija un proveedor de VPS](#elija-un-proveedor-de-vps)
  * [Obtén un plan acorde a tus necesidades](#obtén-un-plan-acorde-a-tus-necesidades)
  * [Elije un sistema operativo](#elije-un-sistema-operativo)
    * [¿Qué es un sistema operativo con panel?](#qué-es-un-sistema-operativo-con-panel)
  * [Configurar contraseña y SSH](#configurar-contraseña-y-ssh)
  * [Agregar una nueva cuenta de usuario](#agregar-una-nueva-cuenta-de-usuario)
  * [Apuntar un registro DNS al servidor (Obtener un dominio)](#apuntar-un-registro-dns-al-servidor-obtener-un-dominio)
  * [Agregar seguridad al VPS](#agregar-seguridad-al-vps)
    * [¿Por qué?](#por-qué)
    * [Asegúrate de que tu usuario no root tenga tu clave pública](#asegúrate-de-que-tu-usuario-no-root-tenga-tu-clave-pública)
    * [Deshabilitar el inicio de sesión con contraseña como usuario root](#deshabilitar-el-inicio-de-sesión-con-contraseña-como-usuario-root)
...
## Elija un proveedor de VPS

Al elegir un proveedor de VPS, descubrirá rápidamente que no faltan opciones. Sin embargo, tres nombres que encontrará con frecuencia en reseñas, foros y recomendaciones son:
...
Elegí **Hostinger** porque dos de mis creadores de contenido de programación favoritos lo usaron en el pasado.
...
## Obtén un plan acorde a tus necesidades
...
Puedes usar mi código de cupón de creador de contenido con descuento... No, no estoy patrocinado, aunque me gustaría (Hostinger, ¡contáctame!)
- Consejo: En YouTube, busca "Hostinger VPS", ordene por fecha para obtener videos recientes y, probablemente, si el video es una buena reseña de un creador con muchos suscriptores, es posible que tengan un código de cupón que pueda usar. Un descuento del 10% siempre es bienvenido.

## Elije un sistema operativo

Después de obtener la instancia, es hora de elegir un sistema operativo: la opción popular sería solo Ubuntu, pero para esta serie de blogs vamos a optar por un sistema operativo con panel -> [Coolify](https://www.coolify.io/).

![hostinger-select-os](/uploads/2025-01-28-vps-2/hostinger-select-os.png)

### ¿Qué es un sistema operativo con panel?
Un sistema operativo estándar (como Ubuntu) te ofrece un servidor en blanco para configurar a través de la línea de comandos. Pero un sistema operativo con panel incluye un panel de control gráfico (como Coolify) que te permite administrar tu servidor a través de una interfaz web simple.
- Piensa en él como un panel de control para tu VPS: puedes implementar aplicaciones, monitorear recursos, configurar ajustes y manejar actualizaciones sin necesidad de memorizar comandos de terminal.
- Los paneles son perfectos para los desarrolladores que quieren la línea de comandos y centrarse en crear cosas.

_Por cierto, **no** uso Arch_.
![arch](/uploads/2025-01-28-vps-2/arch.jpg)

## Configurar contraseña y SSH

Después de elegir el sistema operativo, se te solicitará que configure una contraseña para el usuario root:
- Agregue también una clave SSH para que luego pueda iniciar sesión exclusivamente a través de SSH.
...
Después de finalizar la configuración, espere unos minutos hasta que se inicie la instancia VPS. Luego, pruebe el inicio de sesión a través de SSH.

```bash
ssh root@vps-instance-ip-address
```

Puede encontrar la dirección IP de su instancia VPS en la pantalla "Overview -> VPS Information".
...
Ya tengo mi VPS, ¿ahora qué?

## Agregar una nueva cuenta de usuario

Por razones prácticas y de seguridad, no se recomienda trabajar directamente como **usuario root** en su VPS. A continuación, se explica por qué es recomendable crear una cuenta de usuario dedicada:
...
2. Otorgar privilegios de sudo para tareas administrativas (`usermod -aG sudo pollito`).
3. Pruebe que funciona cambiando al nuevo usuario (`su - pollito`) y ejecutando un comando sudo de prueba (`sudo ls /`)

## Apuntar un registro DNS al servidor (Obtener un dominio)

Para conectar su VPS a un nombre de dominio, deberá configurar los registros DNS. Puede utilizar un dominio existente que posea o comprar uno nuevo (Hostinger ofrece [registro de dominio](https://hpanel.hostinger.com/domains/domain-checker) si prefiere una solución integral). A continuación, se indica cómo configurarlo:
...
3. **Esperar la propagación:** Los cambios de DNS pueden tardar entre 24 y 48 horas en propagarse a nivel mundial. Puede comprobar el progreso utilizando herramientas como `dig yourdomain.com` o `ping yourdomain.com` (una vez que se complete la propagación, debería resolverse en la IP de su VPS).

**Nota**: Evite agregar registros CNAME o A no relacionados para el dominio raíz durante este proceso, ya que pueden entrar en conflicto con su nueva configuración.

## Agregar seguridad al VPS
...
### Deshabilitar el inicio de sesión con contraseña como usuario root

Lo ideal sería que ahora deshabilitaras el inicio de sesión como root para bloquear tu servidor.
...
- El usuario root sólo puede iniciar sesión usando claves SSH (no contraseñas).
   ```bash
   sudo sed -i -e '/^\(#\|\)PermitRootLogin/s/^.*$/PermitRootLogin without-password/' /etc/ssh/sshd_config
   ```
- Asegúrese de que esté habilitada la autenticación de clave pública, lo cual es necesario para el inicio de sesión basado en clave SSH.
   ```bash
    sudo sed -i -e '/^\(#\|\)PubkeyAuthentication/s/^.*$/PubkeyAuthentication yes/' /etc/ssh/sshd_config
  ```
- Deshabilite la autenticación basada en contraseña, imponiendo el uso de claves SSH para iniciar sesión.
   ```bash
...
Asegúrese de estar en un sistema actualizado antes de continuar.

- Asegúrese de que el sistema conozca las últimas versiones de los paquetes.
   ```bash
   sudo apt update
   ```
- Actualizar todos los paquetes instalados.
   ...
   sudo apt upgrade -y
   ```
Probablemente, verá un mensaje indicando que es necesario reiniciar los servicios y/o contenedores.
- Para asegurarse de que todas las actualizaciones se apliquen correctamente, reinicie el VPS
    ```bash
    sudo reboot
    ```
